...
//        String header = user.getHeader();
        //friendDao.getFriend(getItem(position - 1).getUsername()).getLetter()
        String header = (friendDao.getFriend(user.getUsername())).getLetter();
        if (position == 0 || header != null && !header.equals((friendDao.getFriend((getItem(position - 1)).getUsername())).getLetter())) {
        ...
        if (username.equals(Constant.NEW_FRIENDS_USERNAME)) {
            ...
            //设置用户昵称，采用匹配本地蜂友方式
            holder.nameTextview.setText(friend.getTitle());
            //设置用户头像，已经修改环信封装方法
...
//            String letter = friendDao.getFriend(getItem(i).getUsername()).getLetter();
        ...
        protected synchronized FilterResults performFiltering(CharSequence prefix) {
            ...
            if (prefix == null || prefix.length() == 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    /***获取本地数据库蜂友信息***/
                    String username = friendDao.getFriend(user.getUsername()).getTitle();
//                    String username = user.getUsername();
